...
public class MainActivity extends AppCompatActivity implements FrameProcessor {
    ...
    protected void onCreate(Bundle savedInstanceState) {
        ...
        Button toogle = findViewById(R.id.face_detection_camera_toggle_button);
        ...
        bottomSheetButton.setOnClickListener(new View.OnClickListener() {
            @Override
            public void onClick(View view) {
                ImagePicker.with(MainActivity.this).crop().compress(1024).maxResultSize(1000,1000).start();
            }
        });
    ...
    private void analyseImage(Bitmap bitmap) {
        ...
        FirebaseVisionFaceDetectorOptions options = new FirebaseVisionFaceDetectorOptions.Builder().setPerformanceMode(FirebaseVisionFaceDetectorOptions.ACCURATE)
                .setLandmarkMode(FirebaseVisionFaceDetectorOptions.ALL_LANDMARKS).setClassificationMode(FirebaseVisionFaceDetectorOptions.ALL_CLASSIFICATIONS).build();
    ...
    private void detectFaces(List<FirebaseVisionFace> firebaseVisionFaces, Bitmap bitmap) {
        ...
        for(int  i=0;i<firebaseVisionFaces.size();i++)
        {
            ...
            canvas.drawText("Face " + i,(firebaseVisionFaces.get(i).getBoundingBox().centerX()-(firebaseVisionFaces.get(i).getBoundingBox().width() >> 1) + 8f),(firebaseVisionFaces.get(i).getBoundingBox().centerY()+firebaseVisionFaces.get(i).getBoundingBox().height()>>1) - 8f,facepaint);
            ...
            if (face.getLandmark(FirebaseVisionFaceLandmark.RIGHT_EAR) != null)
            {
                ...
                canvas.drawCircle(Objects.requireNonNull(rightEar).getPosition().getX(),rightEar.getPosition().getY(),8f,landmarkPaint);
            }
            ...
            faceDetectionModels.add(new FaceDetectionModel((i+1), "Smiling Probability " + face.getSmilingProbability()));
            faceDetectionModels.add(new FaceDetectionModel((i+1), "Left Eye Open Probability " + face.getLeftEyeOpenProbability()));
            faceDetectionModels.add(new FaceDetectionModel((i+1), "Right Eye Open Probability " + face.getRightEyeOpenProbability()));
